...
سامانه RealRate به یک معماری پیشرفته و کاملاً دانش‌صفر (**Zero-Knowledge Client-Side Encryption**) برای حفاظت بی‌قیدوشرط از اطلاعات مالی مجهز است:
- **معماری دانش‌صفر (Zero-Knowledge Architecture)**: رمز عبور کاربر **هرگز به سرور ارسال نمی‌شود** و در هیچ کجای دیتابیس ذخیره نمی‌گردد. کلید اختصاصی ۲۵۶ بیتی متقارن با الگوریتم استاندارد **PBKDF2** (با ۱۰۰,۰۰۰ دور هش SHA-256) و سالت ۱۶ بایتی تصادفی، منحصراً در مرورگر کاربر تولید و نگهداری می‌شود.
- **رمزنگاری کامل و جایگزینی داده‌های موجود در دیتابیس (Two-Way Data Migration)**:
  - **هنگام فعال‌سازی E2EE**: کلیه دارایی‌ها و تراکنش‌های پورتفو بلافاصله در کلاینت رمزنگاری (`AES-GCM-256`) شده و رکوردهای دیتابیس با ارقام صفر و مقادیر پوششی **جایگزین و بازنویسی (Overwrite)** می‌شوند؛ بنابراین هیچ اثری از داده‌های خام قبلی در دیتابیس باقی نمی‌ماند.
  - **هنگام غیرفعال‌سازی E2EE**: خاموش کردن گاوصندوق بدون ورود رمز عبور معتبر **غیرممکن** است. با تایید رمز، تمام داده‌های رمزشده در مرورگر رمزگشایی شده و به صورت داده‌های عادی در دیتابیس بازنویسی می‌شوند.
- **حفاظت سخت‌گیرانه از حافظه (Memory & Session Guard)**: در زمان قفل بودن گاوصندوق، وضعیت State کلاینت کاملاً خالی (`[]`) است و هیچ اطلاعاتی در رم مرورگر یا کنسول Inspect باقی نمی‌ماند. همچنین رمز عبور موقت صرفاً در `sessionStorage` همان تب نگهداری شده و با بستن تب مرورگر فوراً و برای همیشه نابود می‌گردد.
- **تضمین امنیت در برابر نفوذ سرور**: حتی در فرضی‌ترین حالت هک سرور یا افشای دیتابیس Cloudflare D1، نفوذگر صرفاً با رشته‌های نامفهوم `enc:e2ee:v1:...` و مقادیر صفر مواجه می‌شود که شکستن آن بدون رمز عبور شما از نظر محاسباتی ناممکن است.
- 🔐 [مطالعه مستندات کامل فنی و رمزشناسی گاوصندوق E2EE](docs/E2EE_VAULT.md)

### 🛠️ ۷. مدیریت سورس‌ها با معماری Code-First و قرارداد یکپارچه
- **قرارداد قطعی و یکدست ادپتورها**: تمام ادپتورها بدون استثنا خروجی استاندارد `{ items: [{ id, name, price }], datetime }` بازمی‌گردانند؛ حتی سورس‌های تک‌مقداری آرایه‌ای یک‌عضوی تحویل می‌دهند.
- **موتور مرکزی نمایش (`displayEngine.js`)**: پل مشترک سیم‌لینک‌شده میان بک‌اند و فرانت‌اند برای تولید فرمت یکنواخت نام‌ها `"{نام آیتم} ({نام سورس})"`. واحدهای اندازه‌گیری، بج‌ها، دسته‌بندی‌ها، رنگ‌ها و آیکون‌ها بدون هیچ شرط هاردکدی رندر می‌شوند.
- **مسیر واحد ذخیره‌سازی داده‌ها (`sourceItems.repository.js`)**: جایگزینی مسیرهای پراکنده قدیمی با یک مسیر ذخیره‌سازی همزمان در KV Cache (`source_items:{sourceId}`) و ردیف متناظر در دیتابیس D1.
- **ارکستراسیون تک‌تیک بدون درخواست تکراری (`sourceSync.service.js`)**: پولینگ هوشمند خودکار که اندپوینت‌های مشترک را ادغام کرده و از فراخوانی‌های موازی و تکراری جلوگیری می‌کند.
...
│   ├── ADDING_NEW_PRICE_SOURCE.md       # راهنمای پیاده‌سازی سورس با قرارداد یکپارچه
│   └── EXAMPLE_ADDING_API_SOURCE.md     # نمونه اتصال وب‌سرویس جدید با ISourceAdapter
│
...
│   ├── tests/unit/                      # آزمون‌های واحد و تطابق قراردادها
│   │   ├── adaptersContract.test.js     # تست تطابق قرارداد همه‌ی ادپتورها ({items, datetime})
...
│   │   ├── sourceSync.test.js           # تست ارکستراسیون پولینگ و حذف فراخوانی‌های تکراری
│   │   ├── sourcesValidation.test.js    # اعتبارسنجی کانفیگ منابع
│   │   ├── formulas.test.js             # تست فرمول‌های ریاضی طلا، انس و حباب
│   │   ├── bourseMerge.test.js          # تست ادغام تجمعی داده‌های بورس
│   │   └── unifiedItemsRoute.test.js    # تست کاتالوگ جامع بازار
│   └── src/
│       ├── index.js                     # مدخل ورودی Worker، میدلورها و روتینگ
│       ├── config/                      # مراجع واحد حقیقت (Single Sources of Truth)
│       │   ├── sources.config.js        # رجیستری مرکزی و کانون تعاریف سورس‌ها (Code-First)
│       │   ├── categories.config.js     # تعاریف دسته‌بندی‌ها، بج‌ها، رنگ‌ها و آیکون‌ها
│       │   └── sourceRegistry.js        # هلپرهای جستجو و نگاشت داده‌محور
│       ├── domain/                      # هسته محاسباتی و موتور نمایش
│       │   ├── displayEngine.js         # موتور مرکزی نمایش (Display Engine)
│       │   ├── formulas.js              # فرمول‌های خالص ریاضی طلا و حباب
│       │   └── specs/                   # تعاریف کانونی وزن و عیار مسکوکات و ارزها
│       ├── repositories/                # لایه دسترسی به داده (Repository Pattern)
│       │   ├── sourceItems.repository.js# مسیر واحد ذخیره اقلام منابع در KV و D1
...
│       ├── services/                    # سرویس‌های کاربردی و ارکستراسیون
│       │   └── market/
│       │       ├── sourceSync.service.js# ارکستراتور پولینگ تک‌تیک و همگام‌سازی
│       │       ├── priceAggregator.service.js # تجمیع و محاسبه نرخ‌های بازار
│       │       └── sources/             # ادپتورهای دریافت نرخ (ISourceAdapter)
│       │           ├── ISourceAdapter.js# اینترفیس و قرارداد نهایی ادپتورها
│       │           ├── telegramSource.adapter.js # پارسر تلگرام
│       │           ├── apiUrl.source.adapter.js  # آداپتور عمومی وب‌سرویس‌ها
│       │           ├── bourseSymbols.source.adapter.js # آداپتور بورس تهران
│       │           ├── emofidFunds.source.adapter.js   # صندوق‌های مفید
│       │           ├── charismaFunds.source.adapter.js # صندوق‌های کاریزما
│       │           ├── charismaPlans.source.adapter.js # طرح‌های کاریزما
│       │           └── forexApi.source.adapter.js      # آداپتور نرخ‌های کراس فارکس
│       ├── handlers/                    # کنترلرهای پاسخ‌دهی به درخواست‌های HTTP
│       │   ├── unifiedItemsRoute.js     # کاتالوگ جامع بازار (/api/v1/market/items)
...
│       └── jobs/                        # جاب‌های زمان‌بندی‌شده Worker
│           └── cronPolling.job.js       # جاب کرون پولینگ دوره‌ای
│
└── web/                                 # سرویس فرانت‌اند (React 19 + Vite SPA)
    ...
    └── src/
        ...
        ├── config/
        │   ├── displayEngine.js         # سیم‌لینک موتور نمایش مرکزی به api
        │   ├── sources.config.js        # سیم‌لینک سورس‌های کانونی به api
        │   └── categories.config.js     # سیم‌لینک دسته‌بندی‌های کانونی به api
        ...
        │   └── UniversalAssetSearch.jsx # جستجوگر جامع و داده‌محور تمام اقلام بازار
        ...
        ├── shared/                      # کامپوننت‌ها و ابزارهای مشترک UI
...
- 🔐 [معماری و مستندات فنی گاوصندوق رمزنگاری سرتاسری (E2EE Vault)](docs/E2EE_VAULT.md)
